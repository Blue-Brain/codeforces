var firstPerson = readline();
var secondPerson = readline();
var comboLine = readline();
var result = '';
function findSymbol (symbol, selectedString) {
    var stringCompare = comboLine; 
    comboLine = comboLine.replace(selectedString[symbol], '');
    if (stringCompare===comboLine) {
        result = result + selectedString[symbol];
    }
}  
function selectStringCompare (selectedString) {
    for (var n=0; n<selectedString.length; n++) {
        findSymbol(n, selectedString);
    } 
}
selectStringCompare (firstPerson);
selectStringCompare (secondPerson);
result = result + comboLine;
if (!result) print("YES");
else print("NO");

// Веселая шутка (реализация, сортировка, строки).
