var numberTeams = +readline();
var numberGamesAwayFormInHome = 0;
var listTeams = [];
for (var team=0; team < numberTeams; team++) {
    listTeams.push(readline().split(" "));
    if (listTeams.length > 1) {
        for (var otherTeam=team; otherTeam>0; ){
            otherTeam--;
            if (listTeams[team][0] === listTeams[otherTeam][1]) {
                numberGamesAwayFormInHome++;
            } 
            if (listTeams[team][1] === listTeams[otherTeam][0]) {
                numberGamesAwayFormInHome++;
            }
        }
    }
}
print(numberGamesAwayFormInHome);
